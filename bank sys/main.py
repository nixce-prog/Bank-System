...
from rus import main


def lang_change(root):
    root.destroy()
    main.login()
...
def login():
    ...
    btn_next = Button(root, text='RegMenu', command=lambda: next(root, register)).pack()
    ...
    btn_acc = Button(root, text='Accept', width=10, fg='white', bg='#214c8c', activebackground='#214c8c',
                     font=('Italic', 15), bd=0, highlightthickness=0, command=lambda: next(root, main_menu('Text1121')))
    ...
    btn_lang = Button(root, text='RU', command=lambda: lang_change(root)).pack()
